...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " BITSZ-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  BITSZ-tx [options] <hex-tx> [commands]  Update hex-encoded BITSZ transaction\n" +
            "or:     BITSZ-tx [options] -create [commands]   Create hex-encoded BITSZ transaction\n" +
            "\n";
