...
static const bool DEFAULT_TXINDEX = true;  // BITSZ: txindex is required for PoS calculations (might change in the future)
...
// BITSZ:
CAmount GetProofOfWorkReward(unsigned int nBits, uint32_t nTime, int nHeight);
CAmount GetProofOfStakeReward(int64_t nCoinAge, uint32_t nTime, uint64_t nMoneySupply);
bool GetCoinAge(const CTransaction& tx, const CCoinsViewCache &view, uint64_t& nCoinAge, unsigned int nTimeTx, bool isTrueCoinAge = true); // BITSZ: get transaction coin age
