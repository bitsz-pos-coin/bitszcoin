...
// BITSZ: if pwallet != NULL it will attempt to create coinstake
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool* pfPoSCancel)
{
    ...
    if (pwallet == nullptr) {
        ...
        coinbaseTx.vout[0].nValue = GetProofOfWorkReward(pblock->nBits, pblock->nTime, nHeight);
        }
    ...
    // BITSZ: if coinstake available add coinstake tx
    ...
    if (pwallet)  // attemp to find a coinstake
    {
        *pfPoSCancel = true;
        ...
        if (nSearchTime > nLastCoinStakeSearchTime)
        {
            ...
            {
                if (txCoinStake.nTime >= std::max(pindexPrev->GetMedianTimePast()+1, pindexPrev->GetBlockTime() - (IsProtocolV09(pindexPrev->GetBlockTime()) ? MAX_FUTURE_BLOCK_TIME : MAX_FUTURE_BLOCK_TIME)))
        ...
        if (*pfPoSCancel)
            return nullptr; // BITSZ: there is no point to continue if we failed to create coinstake
    ...
    pblock->nTime          = std::max(pblock->GetBlockTime(), pindexPrev->GetBlockTime() - (IsProtocolV09(pindexPrev->GetBlockTime()) ? MAX_FUTURE_BLOCK_TIME : MAX_FUTURE_BLOCK_TIME));
...
static bool ProcessBlockFound(const CBlock* pblock, const CChainParams& chainparams)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != ::ChainActive().Tip()->GetBlockHash())
            return error("BITSZMiner: generated block is stale");
    }
...
void PoSMiner(std::shared_ptr<CWallet> pwallet, CConnman* connman, CTxMemPool* mempool)
{
    ...
    util::ThreadRename("BITSZ-stake-minter");
    ...
    try {
        ...
        while (true) {
...

            {
                ...
                {
                    LogPrintf("BITSZMiner runtime error: %s\n", e.what());
                    continue;
                }
            }
            ...
            if (!pblocktemplate.get())
            {
                ...
                uiInterface.NotifyAlertChanged(uint256(), CT_UPDATED);
                LogPrintf("Error in BITSZMiner: Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                if (!connman->interruptNet.sleep_for(std::chrono::seconds(10)))
                   return;
            ...
            // BITSZ: if proof-of-stake block found then process block
            if (pblock->IsProofOfStake())
            {
                {
                    LOCK2(cs_main, pwallet->cs_wallet);
    ...
    {
        LogPrintf("BITSZMiner terminated\n");
        return;
    }
    catch (const std::runtime_error &e)
    {
...
{
    // BITSZ: mint proof-of-stake blocks in the background
