...
void BitcoinGUI::onResult(QNetworkReply *reply) {
    if(reply->error() == QNetworkReply::NoError) {
        std::regex versionRgx("v([0-9]+).([0-9]+).([0-9]+)BITSZ");
        std::smatch matches;
        int newVersion=0;
...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a BITSZ address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and BITSZ: URIs)"));
...
*/
    ...
    connect(mintingAction, SIGNAL(triggered()), this, SLOT(gotoStakingPage()));
...
//    connect(multisigAction, SIGNAL(triggered()), this, SLOT(gotoMultisigPage()));
    ...
    decryptForStakingAction->setStatusTip(tr("Decrypt wallet only for minting. Sending coins will still require the password."));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your BITSZ addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify Message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified BITSZ addresses"));
    ...
    openAction->setStatusTip(tr("Open a BITSZ: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible BITSZ command-line options").arg(PACKAGE_NAME));

    openWebAction = new QAction(tr("&Website"), this);
    openWebAction->setStatusTip(tr("Open the BITSZ website in a web browser."));
...
void BitcoinGUI::openWeb() {
    QDesktopServices::openUrl(QUrl("https://Bitsz.io"));
}
#endif // ENABLE_WALLET
...
void BitcoinGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        tooltip = tr("%n active connection(s) to BITSZ network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void BitcoinGUI::setEncryptionStatus(int status)
{
    ...
    case WalletModel::Unlocked:
        ...
        decryptForStakingAction->setEnabled(fWalletUnlockMintOnly);
        decryptForStakingAction->setChecked(fWalletUnlockMintOnly);
        break;
