...
{
public:
    StakingTablePriv(WalletModel *walletModel, StakingTableModel *parent):
    ...
    {
        {
            return TransactionDesc::toHTML(walletModel->node(), walletModel->wallet(), rec, BitcoinUnits::BITSZ);  
        }
        return QString("");
    }
...
static void NotifyTransactionChanged(StakingTableModel *ttm, const uint256 &hash, ChangeType status)
{
    // Find transaction in wallet
...
StakingTableModel::StakingTableModel(WalletModel *parent) :
        QAbstractTableModel(parent),
        walletModel(parent),
        mintingInterval(1440),
        priv(new StakingTablePriv(walletModel, this)),
        cachedNumBlocks(0)
{
...
void StakingTableModel::updateTransaction(const QString &hash, int status)
{
    uint256 updated;
...
void StakingTableModel::updateAge()
{
...
void StakingTableModel::setStakingProxyModel(StakingFilterProxy *mintingProxy)
{
...
double StakingTableModel::getDayToMint(KernelRecord *wtx) const
{
...
QString StakingTableModel::formatDayToMint(KernelRecord *wtx) const
{
    double prob = getDayToMint(wtx);
    return QString::number(prob, 'f', 6) + "%";
}

QString StakingTableModel::formatTxAddress(const KernelRecord *wtx, bool tooltip) const
{
    return QString::fromStdString(wtx->address);
}

QString StakingTableModel::formatTxHash(const KernelRecord *wtx) const
{
    return QString::fromStdString(wtx->hash.ToString());
}

QString StakingTableModel::formatTxCoinDay(const KernelRecord *wtx) const
{
    return QString::number(wtx->getCoinAge());
}

QString StakingTableModel::formatTxAge(const KernelRecord *wtx) const
{
    int64_t nAge = wtx->getAge();
    return QString::number(nAge);
}

QString StakingTableModel::formatTxBalance(const KernelRecord *wtx) const
{
...
void StakingTableModel::updateDisplayUnit()
{
