 ...
 */
static const char* BITCOIN_PID_FILENAME = "BITSZd.pid";
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/BITSZ/BITSZ>";
...
/** Sanity checks
 *  Ensure that BITSZ is running in a usable environment with all
...
{
    // Make sure only a single BITSZ process is using the data directory.
...
bool AppInitSanityChecks()
{
    ...
    // BITSZ: init hash seed
    BITSZRandseed = GetRand(1 << 30);
...
bool AppInitMain(NodeContext& node)
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if BITSZ is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if BITSZ is started while in a temporary directory.\n",
        ...
        do {
            ...
            try {
                ...
                if (!is_coinsview_empty) {
                    ...
                    if (tip && tip->nTime > GetAdjustedTime() + 24 * 60 * 60) {
