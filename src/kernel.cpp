...
const uint32_t nBITSZ16BIPsSwitchTime           = 1569931200; // Tue 01 Oct 12:00:00 UTC 2019
const uint32_t nBITSZ16BIPsTestSwitchTime       = 1554811200; // Tue 09 Apr 12:00:00 UTC 2019
...
const unsigned int nProtocolV10SwitchTime     = 1652651269;
const unsigned int nProtocolV10TestSwitchTime = 1652651269;
...
bool IsBITSZ16BIPsEnabled(uint32_t nTimeTx)
{
    bool fTestNet = Params().NetworkIDString() != CBaseChainParams::MAIN;
    return (nTimeTx >= (fTestNet? nBITSZ16BIPsTestSwitchTime : nBITSZ16BIPsSwitchTime));
}
...
bool CheckStakeKernelHash(unsigned int nBits, CBlockIndex* pindexPrev, const CBlockHeader& blockFrom, unsigned int nTxPrevOffset, const CTransactionRef& txPrev, const COutPoint& prevout, unsigned int nTimeTx, uint256& hashProofOfStake, bool fPrintProofOfStake)
{
    ...
    CBigNum bnCoinDayWeight = CBigNum(nValueIn) * nTimeWeight / COIN / (1 * 60 * 60);
    // Calculate hash
    CDataStream ss(SER_GETHASH, 0);
...
// BITSZ: entropy bit for stake modifier if chosen by modifier
