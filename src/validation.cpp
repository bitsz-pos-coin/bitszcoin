...
#if defined(NDEBUG)
# error "BITSZ cannot be compiled without assertions."
#endif
...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    if (tx.nTime > GetAdjustedTime() + (IsProtocolV09(GetAdjustedTime()) ? MAX_FUTURE_BLOCK_TIME : MAX_FUTURE_BLOCK_TIME))
...
int64_t GetProofOfWorkReward(unsigned int nBits, uint32_t nTime, int nHeight)
{
    if (nHeight <= 1000)
        return 150000 * COIN;
...
{
    if (fUpdateUI)
        uiInterface.NotifyAlertChanged(uint256(), CT_UPDATED); // BITSZ: we are using arguments that will have no effects in updateAlert()
#if HAVE_SYSTEM
...
int ApplyTxInUndo(Coin&& undo, CCoinsViewCache& view, const COutPoint& out)
{
    ...
    if (undo.nHeight == 0) {
        ...
        if (!alternate.IsSpent()) {
            ...
            undo.fCoinStake = alternate.fCoinStake; // BITSZ
            undo.nTime = alternate.nTime;           // BITSZ
        } else {
...
static unsigned int GetBlockScriptFlags(const CBlockIndex* pindex, const Consensus::Params& consensusparams) EXCLUSIVE_LOCKS_REQUIRED(cs_main) {
    ...
    if (pindex->pprev && IsBITSZ16BIPsEnabled(pindex->pprev->nTime)) {
...
bool BITSZContextualBlockChecks(const CBlock& block, BlockValidationState& state, CBlockIndex* pindex, bool fJustCheck)
{
    uint256 hashProofOfStake = uint256();
    // BITSZ: verify hash target and signature of coinstake tx
    ...
    // BITSZ: compute stake entropy bit for stake modifier
    unsigned int nEntropyBit = GetStakeEntropyBit(block);

    // BITSZ: compute stake modifier
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    if (pindex->nStakeModifier == 0 && pindex->nStakeModifierChecksum == 0 && !BITSZContextualBlockChecks(block, state, pindex, fJustCheck))
    ...
    // BITSZ: coinbase reward check relocated to CheckBlock()
    ...
    // BITSZ: track money supply and mint amount info
    pindex->nMint = nValueOut - nValueIn + nFees;
    pindex->nMoneySupply = (pindex->pprev? pindex->pprev->nMoneySupply : 0) + nValueOut - nValueIn;

    // BITSZ: fees are not collected by miners as in bitcoin
    // BITSZ: fees are destroyed to compensate the entire network
...
bool CheckBlock(const CBlock& block, BlockValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSignature)
{
    ...
    // BITSZ: only the second transaction can be the optional coinstake
    for (unsigned int i = 2; i < block.vtx.size(); i++)
        if (block.vtx[i]->IsCoinStake())
            return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-cs-missing", "coinstake in wrong position");

    // BITSZ: first coinbase output should be empty if proof-of-stake block
    ...
    if (block.GetBlockTime() > (block.vtx[0]->nTime ? (int64_t)block.vtx[0]->nTime : block.GetBlockTime()) + (IsProtocolV09(block.GetBlockTime()) ? MAX_FUTURE_BLOCK_TIME : MAX_FUTURE_BLOCK_TIME))
    ...
    // Check POW period
    if (block.IsProofOfWork() && IsProtocolV10(block.GetBlockTime()))
        return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-pow", "POW period ended.");
    ...
    if (block.vtx[0]->GetValueOut() > (block.IsProofOfWork()? (GetProofOfWorkReward(block.nBits, block.GetBlockTime(), nHeight) - nCoinbaseCost) : 0))
        return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-cb-amount",
                strprintf("CheckBlock() : coinbase reward exceeded %s > %s",
                   FormatMoney(block.vtx[0]->GetValueOut()),
                   FormatMoney(block.IsProofOfWork()? GetProofOfWorkReward(block.nBits, block.GetBlockTime(), nHeight) : 0)));
    // Check transactions
    ...
    for (const auto& tx : block.vtx) {
        ...
        if (!CheckTransaction(*tx, tx_state)) {
            ...
            // BITSZ: check transaction timestamp
    ...
    if (fCheckMerkleRoot && fCheckSignature && (block.IsProofOfStake() || !IsBITSZ16BIPsEnabled(block.GetBlockTime())) && !CheckBlockSignature(block))
...
{
    return pindexPrev ? IsBITSZ16BIPsEnabled(pindexPrev->nTime) : false; // pindexPrev == null on genesis block
}
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, BlockValidationState& state, const CChainParams& params, const CBlockIndex* pindexPrev, int64_t nAdjustedTime) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    if (block.GetBlockTime() <= pindexPrev->GetMedianTimePast() || block.GetBlockTime() + (IsProtocolV09(block.GetBlockTime()) ? MAX_FUTURE_BLOCK_TIME : MAX_FUTURE_BLOCK_TIME) < pindexPrev->GetBlockTime())
        return state.Invalid(BlockValidationResult::BLOCK_INVALID_HEADER, "time-too-old", "block's timestamp is too early");

    // Check timestamp
    if (block.GetBlockTime() > nAdjustedTime + (IsProtocolV09(block.GetBlockTime()) ? MAX_FUTURE_BLOCK_TIME : MAX_FUTURE_BLOCK_TIME))
...
static bool ContextualCheckBlock(const CBlock& block, BlockValidationState& state, const CBlockIndex* pindexPrev)
{
    ...
    int nLockTimeFlags = 0;
    if (pindexPrev && IsBITSZ16BIPsEnabled(pindexPrev->nTime)) {
        assert(pindexPrev != nullptr);
...
bool CChainState::AcceptBlock(const std::shared_ptr<const CBlock>& pblock, BlockValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex, bool fRequested, const FlatFilePos* dbp, bool* fNewBlock)
{
    ...
    // BITSZ: we should only accept blocks that can be connected to a prev block with validated PoS
    ...
    // BITSZ: check PoS
    if (fCheckPoS && !BITSZContextualBlockChecks(block, state, pindex, false)) {
        pindex->nStatus |= BLOCK_FAILED_VALID;
        setDirtyBlockIndex.insert(pindex);
    ...
    std::set<CBlockIndex*, CBlockIndexWorkComparator>& block_index_candidates)
{
    ...
    for (const std::pair<int, CBlockIndex*>& item : vSortedByHeight)
    {
        ...
        // BITSZ: calculate stake modifier checksum
...
// BITSZ: total coin age spent in transaction, in the unit of coin-days.
...
bool GetCoinAge(const CTransaction& tx, const CCoinsViewCache &view, uint64_t& nCoinAge, unsigned int nTimeTx, bool isTrueCoinAge)
{
    ...
    arith_uint256 bnCoinDay = bnCentSecond * CENT / COIN / (1 * 60 * 60);
    if (gArgs.GetBoolArg("-printcoinage", false))
