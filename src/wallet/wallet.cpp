...
// BITSZ: optional setting to unlock wallet for block minting only;
...
{
    // BITSZ: we still have the timestamp, so use it to avoid confusion
    if (tx->nTime)
        return tx->nTime;
...
void CWallet::AvailableCoins(interfaces::Chain::Lock& locked_chain, std::vector<COutput>& vCoins, bool fOnlySafe, const CCoinControl* coinControl, const uint32_t nSpendTime, const CAmount& nMinimumAmount, const CAmount& nMaximumAmount, const CAmount& nMinimumSumAmount, const uint64_t nMaximumCount) const
{
    ...
    for (const auto& entry : mapWallet)
    {
        ...
        if (nSpendTime > 0 && wtx.tx->nTime > nSpendTime)
            continue;  // BITSZ: timestamp must not exceed spend time
...
//            coin_selection_params.use_bnb = false;
            ...
            while (true)
            {
                ...
                // BITSZ: sub-cent change is moved to fee
                ...
                if (nFeeRet >= nFeeNeeded) {
                    ...
                    if (nChangePosInOut == -1 && nSubtractFeeFromAmount == 0 && pick_new_inputs) {
                        ...
                        CAmount minimum_value_for_change = MIN_TXOUT_AMOUNT;
...
// BITSZ: create coin stake transaction
