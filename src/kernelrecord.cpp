...
int64_t KernelRecord::getCoinAge() const
{
    ...
    int nDayWeight = (min((GetAdjustedTime() - nTime), params.nStakeMaxAge) - params.nStakeMinAge) / 3600;
...
double KernelRecord::getProbToMintStake(double difficulty, int timeOffset) const
{
    ...
    int dayWeight = (min((GetAdjustedTime() - nTime) + timeOffset, params.nStakeMaxAge) - params.nStakeMinAge) / 3600;
...
double KernelRecord::getProbToMintWithinNMinutes(double difficulty, int minutes)
{
    if(difficulty != prevDifficulty || minutes != prevMinutes)
    {
        ...
        {
            timeOffset = i * 3600;
            p = pow(1 - getProbToMintStake(difficulty, timeOffset), 3600);
            prob *= p;
        }
