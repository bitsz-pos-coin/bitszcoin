...
// BITSZ: get network Gh/s estimate
...
static UniValue generateBlocks(const CTxMemPool& mempool, const CScript& coinbase_script, int nGenerate, uint64_t nMaxTries, const CWallet * const pwallet)
{
    ...
    while (nHeight < nHeightEnd && !ShutdownRequested())
    {
        ...
        if (!IsBITSZ16BIPsEnabled(pblock->GetBlockTime()) && !SignBlock(*pblock, *pwallet))
...
static UniValue generatetoaddress(const JSONRPCRequest& request)
{
            RPCHelpMan{"generatetoaddress",
                "\nMine blocks immediately to a specified address (before the RPC call returns)\n",
                {
                    ...
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The address to send the newly generated BITSZ to."},
                    {"maxtries", RPCArg::Type::NUM, /* default */ "1000000", "How many iterations to try."},
                },
                RPCResult{
                    RPCResult::Type::ARR, "", "hashes of blocks generated",
            ...
            + "If you are running the BITSZ wallet, you can get a new address to send the newly generated BITSZ to with:\n"
...
static UniValue submitblock(const JSONRPCRequest& request)
{
    ...
    // BITSZ: check block before attempting to sign it
    BlockValidationState state;
    ...
    if ((block.IsProofOfStake() || !IsBITSZ16BIPsEnabled(block.GetBlockTime())) && !SignBlock(block, *pwallet))
        throw JSONRPCError(-100, "Unable to sign block, wallet locked?");
